...
import { DEFAULT_CROP, deriveProblems, type Marker } from '../lib/problembook.ts';
...
test('다음 단까지 이어지는 문제를 여러 조각으로 묶는다', () => {
  ...
  assert.deepEqual(result.problems[0].fragments[1], { page: 1, column: 'right', yStart: 0.04, yEnd: 0.3 });
});
...
test('Start 없는 End를 경고한다', () => {
  const result = deriveProblems([marker('a', 1, 'left', 0.5, 'end')], 1, crop);
